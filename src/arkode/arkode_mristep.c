...
int mriStep_TakeStep(void* arkode_mem, realtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                     "ARKODE::mriStep_TakeStep", "slow stage", "z[0] =", "");
  ...
  if (step_mem->explicit_rhs)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "Fse[0] =", "");
    SUNCheckCallLastErrNoRet(
      N_VPrintFile(step_mem->Fse[0], ark_mem->sunctx->logger->debug_fp));
  }
  if (step_mem->implicit_rhs)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "Fsi[0] =", "");
    SUNCheckCallLastErrNoRet(
      N_VPrintFile(step_mem->Fsi[0], ark_mem->sunctx->logger->debug_fp));
  }
#endif
  ...
  if (step_mem->NLS)
  {
    ...
    {
      SUNCheckCallLastErrNoRet(N_VConst(ZERO, ark_mem->tempv3)); /* set guess to
                                                                    0 for
                                                                    predictor-corrector
                                                                    form */
      nls_status = SUNCheckCallLastErrNoRet(
        SUNNonlinSolSetup(step_mem->NLS, ark_mem->tempv3, ark_mem));
                                ...
                                int stage, realtype cdiff)
{
  ...
  for (k = 0; k < nmat; k++)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "forcing[%i] =", k);
    SUNCheckCallLastErrNoRet(N_VPrintFile(step_mem->stepper->forcing[k],
                                          ark_mem->sunctx->logger->debug_fp));
  }
#endif
