                                     ...
                                     MRIStepInnerStepper *stepper)
{
  ...
  retval = erkStep_AccessStepMem(inner_arkode_mem,
                                 "ERKStepCreateMRIStepInnerStepper",
                                 &ark_mem, &step_mem);
  if (retval) {
    arkProcessError(NULL, ARK_ILL_INPUT, "ARKODE::ERKStep",
...
int erkStep_MRIStepInnerEvolve(MRIStepInnerStepper stepper, realtype t0,
...
int erkStep_MRIStepInnerSetRTol(MRIStepInnerStepper stepper, realtype rtol)
{
  void* arkode_mem;
  ARKodeMem ark_mem;
  int   retval;
  ...
  if (arkode_mem==NULL) {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ERKStep",
                    "erkStep_MRIStepInnerSetRTol", MSG_ARK_NO_MEM);
    return(ARK_MEM_NULL);
  }
  ark_mem = (ARKodeMem) arkode_mem;
