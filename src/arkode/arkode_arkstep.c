...
int arkStep_TakeStep_Z(void* arkode_mem, realtype* dsmPtr, int* nflagPtr)
{
  ...
  if (step_mem->NLS)
  {
    if ((step_mem->NLS)->ops->setup)
    {
      ...
      SUNCheckCallLastErrNoRet(N_VConst(ZERO, zcor0)); /* set guess to all 0
                                                          (since using
                                                          predictor-corrector
                                                          form) */
      nls_status = SUNCheckCallLastErrNoRet(
        SUNNonlinSolSetup(step_mem->NLS, zcor0, ark_mem));
  ...
  for (is = 0; is < step_mem->stages; is++)
  {
    ...
    SUNCheckCallLastErrNoRet(
      N_VPrintFile(step_mem->zpred, ark_mem->sunctx->logger->debug_fp));
#endif
    ...
    SUNCheckCallLastErrNoRet(
      N_VPrintFile(step_mem->sdata, ark_mem->sunctx->logger->debug_fp));
#endif
    ...
    if (step_mem->implicit)
    {
      ...
      SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                         ...
                         "Fi[%i] =", is);
      SUNCheckCallLastErrNoRet(
        N_VPrintFile(step_mem->Fi[is], ark_mem->sunctx->logger->debug_fp));
#endif
    ...
    if (step_mem->explicit)
    {
      ...
      SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                         ...
                         "Fe[%i] =", is);
      SUNCheckCallLastErrNoRet(
        N_VPrintFile(step_mem->Fe[is], ark_mem->sunctx->logger->debug_fp));
#endif
...
int arkStep_StageSetup(ARKodeMem ark_mem, booleantype implicit)
{
  ...
  if (implicit && (step_mem->predictor == 5))
  {
    ...
    else { SUNCheckCallLastErrNoRet(N_VConst(ZERO, step_mem->sdata)); }
    return (ARK_SUCCESS);
  }
  ...
  *dsmPtr = ZERO;
  ...
  if (retval < 0)
  {
    *dsmPtr = RCONST(2.0); /* indicate too much error, step with smaller step */
    SUNCheckCallLastErrNoRet(N_VScale(ONE, ark_mem->yn, y)); /* place old
                                                                solution into y
                                                              */
    return (CONV_FAIL);
  }
