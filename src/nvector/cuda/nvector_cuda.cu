...
static int GetKernelParameters(N_Vector v, booleantype reduction, size_t& grid,
                               size_t& block, size_t& shMemSize,
                               cudaStream_t& stream, bool& atomic,
                               size_t n = 0);
...
#define NVEC_CUDA_HBUFFERp(x) \
  ((realtype*)NVEC_CUDA_PRIVATE(x)->reduce_buffer_host->ptr)
#define NVEC_CUDA_DBUFFERp(x) \
  ((realtype*)NVEC_CUDA_PRIVATE(x)->reduce_buffer_dev->ptr)
...
struct _N_PrivateVectorContent_Cuda {
  ...
  // reduction workspace
  SUNMemory device_counter;     // device memory for a counter (used in LDS
                                // reductions)
                      ...
                      SUNContext sunctx)
{
  ...
  NVEC_CUDA_CONTENT(v)->host_data =
      SUNMemoryHelper_Wrap(h_vdata, SUNMEMTYPE_HOST);
  NVEC_CUDA_CONTENT(v)->device_data =
      SUNMemoryHelper_Wrap(d_vdata, SUNMEMTYPE_DEVICE);
  NVEC_CUDA_CONTENT(v)->stream_exec_policy =
      DEFAULT_STREAMING_EXECPOLICY.clone();
  NVEC_CUDA_CONTENT(v)->reduce_exec_policy =
      DEFAULT_REDUCTION_EXECPOLICY.clone();
...
void N_VSetHostArrayPointer_Cuda(realtype* h_vdata, N_Vector v)
{
  if (N_VIsManagedMemory_Cuda(v))
  {
    ...
    {
      NVEC_CUDA_CONTENT(v)->host_data =
          SUNMemoryHelper_Wrap((void*)h_vdata, SUNMEMTYPE_UVM);
      NVEC_CUDA_CONTENT(v)->device_data =
          SUNMemoryHelper_Alias(NVEC_CUDA_CONTENT(v)->host_data);
    }
  }
  else
  {
    ...
    {
      NVEC_CUDA_CONTENT(v)->host_data =
          SUNMemoryHelper_Wrap((void*)h_vdata, SUNMEMTYPE_HOST);
    }
  }
}
...
void N_VSetDeviceArrayPointer_Cuda(realtype* d_vdata, N_Vector v)
{
  if (N_VIsManagedMemory_Cuda(v))
  {
    ...
    {
      NVEC_CUDA_CONTENT(v)->device_data =
          SUNMemoryHelper_Wrap((void*)d_vdata, SUNMEMTYPE_UVM);
      NVEC_CUDA_CONTENT(v)->host_data =
...
void N_VLinearSum_Cuda(realtype a, N_Vector X, realtype b, N_Vector Y,
                       N_Vector Z)
{
  size_t grid, block, shMemSize;
  cudaStream_t stream;
...
realtype N_VDotProd_Cuda(N_Vector X, N_Vector Y)
{
  ...
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VDotProd_Cuda: InitializeReductionBuffer "
                         "returned nonzero\n");
  }

  if (atomic)
  {
    dotProdKernel<realtype, sunindextype, GridReducerAtomic>
  ...
  {
    dotProdKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(Y),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
...
realtype N_VMaxNorm_Cuda(N_Vector X)
{
  ...
  if (InitializeReductionBuffer(X, gpu_result, buffer_size))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VMaxNorm_Cuda: InitializeReductionBuffer "
                         "returned nonzero\n");
  }

  if (atomic)
  {
    maxNormKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             nullptr);
  ...
  {
    maxNormKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             NVEC_CUDA_DCOUNTERp(X));
...
realtype N_VWSqrSumLocal_Cuda(N_Vector X, N_Vector W)
{
  ...
  if (GetKernelParameters(X, true, grid, block, shMemSize, stream, atomic))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VWSqrSumLocal_Cuda: GetKernelParameters "
                         "returned nonzero\n");
  }
  ...
  if (atomic)
  {
    wL2NormSquareKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(W),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
  ...
  {
    wL2NormSquareKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(W),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
...
realtype N_VWSqrSumMaskLocal_Cuda(N_Vector X, N_Vector W, N_Vector Id)
{
  ...
  if (atomic)
  {
    wL2NormSquareMaskKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(W),
                                             NVEC_CUDA_DDATAp(Id),
                                             NVEC_CUDA_DBUFFERp(X),
  ...
  {
    wL2NormSquareMaskKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(W),
                                             NVEC_CUDA_DDATAp(Id),
                                             NVEC_CUDA_DBUFFERp(X),
...
realtype N_VMin_Cuda(N_Vector X)
{
  ...
  if (atomic)
  {
    findMinKernel<realtype, sunindextype, GridReducerAtomic>
  ...
  {
    findMinKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(gpu_result, NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             NVEC_CUDA_DCOUNTERp(X));
...
realtype N_VL1Norm_Cuda(N_Vector X)
{
  ...
  if (InitializeReductionBuffer(X, gpu_result, buffer_size))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VL1Norm_Cuda: InitializeReductionBuffer "
                         "returned nonzero\n");
  }

  if (atomic)
  {
    L1NormKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             nullptr);
  ...
  {
    L1NormKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             NVEC_CUDA_DCOUNTERp(X));
...
booleantype N_VInvTest_Cuda(N_Vector X, N_Vector Z)
{
  ...
  if (InitializeReductionBuffer(X, gpu_result, buffer_size))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VInvTest_Cuda: InitializeReductionBuffer "
                         "returned nonzero\n");
  }

  if (atomic)
  {
    invTestKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(Z),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
  ...
  {
    invTestKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(Z),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
...
booleantype N_VConstrMask_Cuda(N_Vector C, N_Vector X, N_Vector M)
{
  ...
  if (atomic)
  {
    constrMaskKernel<realtype, sunindextype, GridReducerAtomic>
  ...
  {
    constrMaskKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(NVEC_CUDA_DDATAp(C),
                                             NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DDATAp(M),
                                             NVEC_CUDA_DBUFFERp(X),
...
realtype N_VMinQuotient_Cuda(N_Vector num, N_Vector denom)
{
  ...
  if (atomic)
  {
    minQuotientKernel<realtype, sunindextype, GridReducerAtomic>
  ...
  {
    minQuotientKernel<realtype, sunindextype, GridReducerLDS>
        <<<grid, block, shMemSize, stream>>>(gpu_result, NVEC_CUDA_DDATAp(num),
                                             NVEC_CUDA_DDATAp(denom),
                                             NVEC_CUDA_DBUFFERp(num),
                                             NVEC_CUDA_CONTENT(num)->length,
...
int N_VLinearCombination_Cuda(int nvec, realtype* c, N_Vector* X, N_Vector z)
{
  ...
  linearCombinationKernel<<<grid, block, shMemSize, stream>>>(nvec, cdata,
                                                              xdata,
                                                              NVEC_CUDA_DDATAp(
                                                                  z),
                                                              NVEC_CUDA_CONTENT(
                          ...
                          N_Vector* Z)
{
  ...
  if (GetKernelParameters(x, false, grid, block, shMemSize, stream))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VScaleAddMulti_Cuda: GetKernelParameters "
                         "returned nonzero\n");
    return -1;
  }
...
int N_VDotProdMulti_Cuda(int nvec, N_Vector x, N_Vector* Y, realtype* dots)
{
  ...
  if (GetKernelParameters(x, false, grid, block, shMemSize, stream))
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VDotProdMulti_Cuda: GetKernelParameters "
                         "returned nonzero\n");
    return -1;
  }
  grid = nvec;
  ...
  dotProdMultiKernel<realtype, sunindextype, GridReducerAtomic>
                                 ...
                                 N_Vector* Y, N_Vector* Z)
{
  ...
  linearSumVectorArrayKernel<<<grid, block, shMemSize,
                               stream>>>(nvec, a, xdata, b, ydata, zdata,
...
int N_VScaleVectorArray_Cuda(int nvec, realtype* c, N_Vector* X, N_Vector* Z)
{
  ...
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VScaleVectorArray_Cuda: FusedBuffer_Init "
                         "returned nonzero\n");
    return -1;
  }
...
int N_VConstVectorArray_Cuda(int nvec, realtype c, N_Vector* Z)
{
  ...
  {
    SUNDIALS_DEBUG_PRINT("ERROR in N_VConstVectorArray_Cuda: FusedBuffer_Init "
                         "returned nonzero\n");
    return -1;
  }
                                ...
                                realtype* norms)
{
  ...
  wL2NormSquareVectorArrayKernel<realtype, sunindextype, GridReducerAtomic>
                                    ...
                                    N_Vector id, realtype* norms)
{
  ...
  wL2NormSquareMaskVectorArrayKernel<realtype, sunindextype, GridReducerAtomic>
      <<<grid, block, shMemSize, stream>>>(nvec, xdata, wdata,
                                           NVEC_CUDA_DDATAp(id),
                                           NVEC_CUDA_DBUFFERp(W[0]),
                                           NVEC_CUDA_CONTENT(W[0])->length);
                                     ...
                                     N_Vector* X, N_Vector** Y, N_Vector** Z)
{
  ...
  scaleAddMultiVectorArrayKernel<<<grid, block, shMemSize,
                                   stream>>>(nvec, nsum, cdata, xdata, ydata,
                                             zdata,
                                         ...
                                         N_Vector** X, N_Vector* Z)
{
  ...
  linearCombinationVectorArrayKernel<<<
      grid, block, shMemSize, stream>>>(nvec, nsum, cdata, xdata, zdata,
                                        NVEC_CUDA_CONTENT(Z[0])->length);
  PostKernelLaunch();
...
int N_VEnableFusedOps_Cuda(N_Vector v, booleantype tf)
{
  ...
  if (tf)
  {
    ...
    v->ops->nvlinearcombinationvectorarray =
        N_VLinearCombinationVectorArray_Cuda;
...
int N_VEnableWrmsNormMaskVectorArray_Cuda(N_Vector v, booleantype tf)
{
  ...
  v->ops->nvwrmsnormmaskvectorarray =
      tf ? N_VWrmsNormMaskVectorArray_Cuda : NULL;
  return 0;
}
...
int N_VEnableScaleAddMultiVectorArray_Cuda(N_Vector v, booleantype tf)
{
  ...
  v->ops->nvscaleaddmultivectorarray =
      tf ? N_VScaleAddMultiVectorArray_Cuda : NULL;
  return 0;
}
...
static int InitializeReductionBuffer(N_Vector v, realtype value, size_t n)
{
  ...
  if (alloc_mem)
  {
    ...
    alloc_fail =
        SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v), &(vcp->reduce_buffer_host),
                              bytes, SUNMEMTYPE_PINNED,
                              (void*)NVEC_CUDA_STREAM(v));
    if (alloc_fail)
    {
      SUNDIALS_DEBUG_PRINT(
          "WARNING in InitializeReductionBuffer: SUNMemoryHelper_Alloc failed "
          "to alloc SUNMEMTYPE_PINNED, using SUNMEMTYPE_HOST instead\n");

      // If pinned alloc failed, allocate plain host memory
      alloc_fail =
          SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v),
                                &(vcp->reduce_buffer_host), bytes,
                                SUNMEMTYPE_HOST, (void*)NVEC_CUDA_STREAM(v));
      if (alloc_fail)
      {
        SUNDIALS_DEBUG_PRINT(
            "ERROR in InitializeReductionBuffer: SUNMemoryHelper_Alloc failed "
            "to alloc SUNMEMTYPE_HOST\n");
      }
    }

    // Allocate device memory
    alloc_fail =
        SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v), &(vcp->reduce_buffer_dev),
                              bytes, SUNMEMTYPE_DEVICE,
                              (void*)NVEC_CUDA_STREAM(v));
    if (alloc_fail)
    {
      SUNDIALS_DEBUG_PRINT(
  ...
  if (!alloc_fail)
  {
    ...
    copy_fail =
        SUNMemoryHelper_CopyAsync(NVEC_CUDA_MEMHELP(v), vcp->reduce_buffer_dev,
                                  vcp->reduce_buffer_host, bytes,
                                  (void*)NVEC_CUDA_STREAM(v));
...
static int CopyReductionBufferFromDevice(N_Vector v, size_t n)
{
  ...
  copy_fail =
      SUNMemoryHelper_CopyAsync(NVEC_CUDA_MEMHELP(v),
...
#error Incompatible precision for CUDA
  ...
  if (alloc_mem)
  {
    // Allocate pinned memory on the host
    alloc_fail =
        SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v), &(vcp->fused_buffer_host),
                              bytes, SUNMEMTYPE_PINNED,
                              (void*)NVEC_CUDA_STREAM(v));
    if (alloc_fail)
    {
    ...
    // Allocate device memory
    alloc_fail =
        SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v), &(vcp->fused_buffer_dev),
                              bytes, SUNMEMTYPE_DEVICE,
                              (void*)NVEC_CUDA_STREAM(v));
    if (alloc_fail)
    {
...
static int FusedBuffer_CopyToDevice(N_Vector v)
{
  ...
  if (copy_fail)
  {
    SUNDIALS_DEBUG_PRINT("ERROR in FusedBuffer_CopyToDevice: "
                         "SUNMemoryHelper_CopyAsync failed\n");
    return -1;
  }
 ...
 * If SUNDIALS_DEBUG_CUDA_LASTERROR is not defined, then the function does
 * nothing. If it is defined, the function will synchronize and check the last
 * CUDA error.
 */
static void PostKernelLaunch()
{
