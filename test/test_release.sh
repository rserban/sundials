...
# run tests
./suntest_tarscript.sh sundials all both each ON DEV OFF $bt

# check return flag
if [ $? -ne 0 ]; then
    echo "FAILED: ./suntest_tarscript.sh sundials all both each ON DEV OFF $bt" | tee -a suntest.log
    exit 1
fi
...
for rt in "${realtype[@]}"; do
    for is in "${indexsize[@]}"; do
        ...
        echo "TEST: ./suntest_xsdk.sh $rt $is both ON DEV OFF $bt"

        # run tests using xSDK CMake options
        ./suntest_xsdk.sh $rt $is both ON DEV OFF $bt

        # check return flag
        if [ $? -ne 0 ]; then
            echo "FAILED: ./suntest_xsdk.sh $rt $is both ON DEV OFF $bt" | tee -a suntest.log
            exit 1
        else
            echo "PASSED"
        fi
