...
int print_timings(SUNProfiler prof)
{
  ...
  // Output timing in CSV format
  flag = SUNProfiler_SetOutputFormat(prof, SUN_OUTPUTFORMAT_CSV);
  if (flag)
  {
    std::cerr << "SUNProfiler_SetOutputFormat returned " << flag << "\n";
    return 1;
  }
  ...
  // Reset output format
  flag = SUNProfiler_SetOutputFormat(prof, SUN_OUTPUTFORMAT_TABLE);
  if (flag)
  {
    std::cerr << "SUNProfiler_SetOutputFormat returned " << flag << "\n";
    return 1;
...
int main()
{
  ...
  std::cout << "\nTest 1: sleep 1s, print table and csv timings \n";
  ...
  std::cout << "\nTest 2: reset, sleep 2s, print csv and table timings\n";
