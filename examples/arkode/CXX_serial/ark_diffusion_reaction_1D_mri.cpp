...
struct UserData
{
  ...
  realtype     du  = RCONST(0.01);   // diffusion coeff for u
  realtype     dv  = RCONST(0.01);   // diffusion coeff for v
  realtype     dw  = RCONST(0.01);   // diffusion coeff for w
};
...
// ODE Rhs functions
static int RhsDiffusion(realtype t, N_Vector y, N_Vector ydot, void *user_data);
static int RhsReaction(realtype t, N_Vector y, N_Vector ydot, void *user_data);
static int RhsDiffusionReaction(realtype t, N_Vector y, N_Vector ydot,
                                void *user_data);

// Rhs Jacobian functions
static int JacDiffusion(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
                        void *user_data, N_Vector tmp1, N_Vector tmp2,
                        N_Vector tmp3);
static int JacReaction(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
                       void *user_data, N_Vector tmp1, N_Vector tmp2,
                       N_Vector tmp3);
static int JacDiffusionReaction(realtype t, N_Vector y, N_Vector fy,
...
int main(int argc, char *argv[])
{
  ...
  cout << endl;
  cout << "1D Diffusion-Reaction (Brusselator) test problem:" << endl;
  ...
  cout << "    du = " << udata.du << endl;
  cout << "    dv = " << udata.dv << endl;
  cout << "    dw = " << udata.dw << endl;
                     ...
                     OutputData *outdata)
{
  ...
  void *arkode_mem = ARKStepCreate(NULL, RhsDiffusionReaction, uopts->T0,
                                   y);
  ...
  retval = ARKStepSetJacFn(arkode_mem, JacDiffusionReaction);
                     ...
                     OutputData *outdata)
{
  ...
  // Initialize matrix and linear solver data structures
  SUNMatrix Af = SUNBandMatrix(udata->NEQ, 3, 3);
  if (check_retval((void *)Af, "SUNBandMatrix", 0)) return 1;

  SUNLinearSolver LSf = SUNLinSol_Band(y, Af);
  ...
  void *arkode_mem = MRIStepCreate(RhsDiffusion, uopts->T0, y, MRISTEP_ARKSTEP,
                                   inner_arkode_mem);
  ...
  case(3):
    ...
    break;
  case(4):
    C = MRIStepCoupling_LoadTable(MRI_GARK_ESDIRK46a);
    if (check_retval((void *)C, "MRIStepCoupling_LoadTable", 0)) return 1;
    break;
  default:
    cerr << "ERROR: invalid method order" << endl;
  ...
  // Specify fast tolerances
  retval = MRIStepSStolerances(arkode_mem, uopts->reltol, uopts->abstol);
  if (check_retval(&retval, "MRIStepSStolerances", 1)) return 1;

  // Initialize matrix and linear solver data structures
  SUNMatrix As = SUNBandMatrix(udata->NEQ, 3, 3);
  if (check_retval((void *)As, "SUNBandMatrix", 0)) return 1;

  SUNLinearSolver LSs = SUNLinSol_Band(y, As);
  ...
  // Set the Jacobian routine
  retval = MRIStepSetJacFn(arkode_mem, JacDiffusion);
  if (check_retval(&retval, "MRIStepSetJacFn", 1)) return 1;

  // Set max number of nonlinear iters
  retval = MRIStepSetMaxNonlinIters(arkode_mem, 10);
...
static int OutputStatsMRI(void *arkode_mem, void* inner_arkode_mem)
{
  ...
  long int nsts, nfs, nstf, nffe, nffi;
  long int nnis, nncs, njes, nnif, nncf, njef;
  ...
  if (check_retval(&retval, "MRIStepGetNonlinSolvStats", 1)) return 1;

  retval = MRIStepGetNumJacEvals(arkode_mem, &njes);
  if (check_retval(&retval, "MRIStepGetNumJacEvals", 1)) return 1;
  ...
  cout << "  Slow Newton iters      = " << nnis << endl;
  cout << "  Slow Newton conv fails = " << nncs << endl;
  cout << "  Slow Jacobian evals    = " << njes << endl;
                    ...
                    OutputData *outdata)
{
  ...
  // Integrator data and settings
  void *arkode_mem = ARKStepCreate(NULL, RhsDiffusion, uopts->T0, y);
  if (check_retval((void *) arkode_mem, "ARKStepCreate", 0)) return 1;

  // attach implicit Euler
  ...
  retval = ARKStepSetTables(arkode_mem, 1, 0, Bs, NULL);
  if (check_retval(&retval, "ARKStepSetTables", 1)) return 1;
  ...
  // Attach matrix and linear solver
  retval = ARKStepSetLinearSolver(arkode_mem, LSs, As);
  if (check_retval(&retval, "ARKStepSetLinearSolver", 1)) return 1;
  ...
  // Set the Jacobian routine
  retval = ARKStepSetJacFn(arkode_mem, JacDiffusion);
  if (check_retval(&retval, "ARKStepSetJacFn", 1)) return 1;
...
static int OutputStatsLT(void *arkode_mem, void* inner_arkode_mem)
{
  ...
  // Get/print slow integrator implicit solver statistics
  retval = ARKStepGetNonlinSolvStats(arkode_mem, &nnis, &nncs);
  if (check_retval(&retval, "ARKStepGetNonlinSolvStats", 1)) return 1;

  retval = ARKStepGetNumJacEvals(arkode_mem, &njes);
  if (check_retval(&retval, "ARKStepGetNumJacEvals", 1)) return 1;
...
static int RhsDiffusion(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  ...
  realtype duconst = udata->du / udata->dx / udata->dx;
  realtype dvconst = udata->dv / udata->dx / udata->dx;
  realtype dwconst = udata->dw / udata->dx / udata->dx;
  ...
  realtype u, v, w, ul, ur, vl, vr, wl, wr;
...
// Compute the diffusion-reaction ODE Rhs
static int RhsDiffusionReaction(realtype t, N_Vector y, N_Vector ydot,
                                void *user_data)
{
  // Access problem data
...
// Compute the diffusion Jacobian
static int JacDiffusion(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
                        void *user_data, N_Vector tmp1, N_Vector tmp2,
                        N_Vector tmp3)
{
  ...
  for (sunindextype i = 1; i < N - 1; i++)
  {
    SM_ELEMENT_B(J,IDX(i,0),IDX(i-1,0)) = duconst;
    SM_ELEMENT_B(J,IDX(i,1),IDX(i-1,1)) = dvconst;
    SM_ELEMENT_B(J,IDX(i,2),IDX(i-1,2)) = dwconst;

    SM_ELEMENT_B(J,IDX(i,0),IDX(i,0)) = -RCONST(2.0) * duconst;
    SM_ELEMENT_B(J,IDX(i,1),IDX(i,1)) = -RCONST(2.0) * dvconst;
    SM_ELEMENT_B(J,IDX(i,2),IDX(i,2)) = -RCONST(2.0) * dwconst;
                                ...
                                N_Vector tmp2, N_Vector tmp3)
{
  ...
  for (sunindextype i = 1; i < N - 1; i++)
  {
    ...
    // All vars wrt u
    SM_ELEMENT_B(J,IDX(i,0),IDX(i,0)) = -RCONST(2.0) * duconst + TWO * u * v - (w + ONE);
    SM_ELEMENT_B(J,IDX(i,1),IDX(i,0)) = w - TWO * u * v;
    SM_ELEMENT_B(J,IDX(i,2),IDX(i,0)) = -w;

    // All vars wrt v
    SM_ELEMENT_B(J,IDX(i,0),IDX(i,1)) = u * u;
    SM_ELEMENT_B(J,IDX(i,1),IDX(i,1)) = -RCONST(2.0) * dvconst - u * u;

    // All vars wrt w
    SM_ELEMENT_B(J,IDX(i,0),IDX(i,2)) = -u;
    SM_ELEMENT_B(J,IDX(i,1),IDX(i,2)) = u;
    SM_ELEMENT_B(J,IDX(i,2),IDX(i,2)) = -RCONST(2.0) * dwconst + (-ONE / ep) - u;
...
static int OpenOutput(N_Vector y, OutputData *outdata)
{
  ...
  outdata->yout.open("diffusion_reaction_1D_mri.out");
  outdata->yout <<  "# title Diffusion-Reaction (Brusselator)" << endl;
